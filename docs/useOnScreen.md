...
A hook to detect if an element is visible within the viewport.

## Arguments

- `ref` (`React.RefObject`): The ref object of the element to detect visibility.
- `rootMargin` (`string`): The margin around the root (default: `"0px"`).

## Returns

- A boolean indicating if the element is visible within the viewport.
